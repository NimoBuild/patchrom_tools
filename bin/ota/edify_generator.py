...
class EdifyGenerator(object):
  ...
  def __init__(self, version, info, more_assert):
    self.script = []
    self.mounts = set()
    self.version = version
    self.info = info
    self.more_assert = more_assert

  def MakeTemporary(self):
    """Make a temporary script object whose commands can latter be
    appended to the parent script with AppendScript().  Used when the
    caller wants to generate script commands out-of-order."""
    x = EdifyGenerator(self.version, self.info, self.more_assert)
  ...
  def FormatPartition(self, partition):
    ...
    if fstab:
      p = fstab[partition]
      if (self.more_assert):
        self.script.append('assert(format("%s", "%s", "%s", "%s", "%s"));' %
                           (p.fs_type, common.PARTITION_TYPES[p.fs_type],
  ...
  def WriteRawImage(self, mount_point, fn):
    ...
    if fstab:
      ...
      if fn == "boot.img" and p.device == "boot":
        ...
        if (self.more_assert):
          self.script.append(
              'assert(write_raw_image(package_extract_file("%(fn)s"), "%(device)s"));'
              % args)
        else:
          self.script.append(
        ...
        if (self.more_assert):
          self.script.append(
              'assert(package_extract_file("%(fn)s", "%(device)s"));' % args)
        else:
          self.script.append(
  ...
  def SetPermissions(self, fn, uid, gid, mode):
    ...
    if (self.more_assert):
      self.script.append('assert(set_perm(%d, %d, 0%o, "%s"));' % (uid, gid, mode, fn))
    else:
  ...
  def SetPermissionsRecursive(self, fn, uid, gid, dmode, fmode):
    ...
    if (self.more_assert):
      self.script.append('assert(set_perm_recursive(%d, %d, 0%o, 0%o, "%s"));'
  ...
  def AddToZip(self, input_zip, output_zip, input_path=None):
    ...
    common.ZipWriteStr(output_zip, "META-INF/com/google/android/update-binary",
                       data, perms=0755)
# xiaoxx adds 
  def AssertSomeHardwareBranch(self, *branchs):
  	"""Assert that the hardware branch version is one of *branchs."""
	cmd = ("assert(" +
		" ||\0".join(['getprop("ro.hardware.bbk") == "%s"' % (b,)
		for b in branchs]) +
		");")
	self.script.append(self._WordWrap(cmd))


  def AssertSomeProductModel(self, *models):
  	"""Assert that the product model is one of *models."""
	cmd = ("assert(" +
		" ||\0".join(['getprop("ro.product.model") == "%s"' % (m,)
		for m in models]) +
		");")
	self.script.append(self._WordWrap(cmd))
# the end
